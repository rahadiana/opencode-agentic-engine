...
export class CacheService {
  ...
  async execute(input: unknown): Promise<Result<unknown>> {
    if (!this.initialized) return { ok: false, error: { code: "NOT_INIT", message: "CacheService not initialized", status: 500 } }
    try {
  ...
  private async process(input: unknown): Promise<unknown> {
    return { service: "CacheService", input, timestamp: new Date().toISOString() }
  }
}
