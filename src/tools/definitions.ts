...
import { getSkillStore, getEpisodicStore, getConfigLoader, getA2AClient, setA2AClient, getA2AServer, setA2AServer } from "../core/shared-instances.js"
...
import type { ConfidenceScore, ScoringSignals } from "../core/confidence-scorer.js"
...
export function buildAllTools(ctx: ToolContext): Record<string, ToolSpec> {
      ...
      for (let i = 0; i < phase.length && !aborted; i += maxParallel) {
        ...
        const promises = chunk.map(async (taskId) => {
          ...
          try {
            ...
            const sessionModelPref = sessionStore.getModelPreference(execCtx.sessionID, role)
            ...
            coordinator.delegate(role, { id: taskId, assignedTo: role, description: task.description, input: task.context ?? task.description, status: 'running' }, execCtx.sessionID, 0, relevantSkills)
            eventBus.emit({ type: 'task.delegated', payload: { sessionID: execCtx.sessionID, taskId, role, description: task.description, delegationDepth: 0 } })
            const agentCtx = {
              systemPrompt: agent.prompt ?? 'You are a ' + role + ' in a software engineering team.',
              sessionId: execCtx.sessionID, role,
            ...
            if (ok) { await coordinator.updateTask(execCtx.sessionID, taskId, 'done', ok); await coordinator.writeSharedMemory('task:' + taskId, ok.slice(0, 500), role) }
  ...
  function evidenceToSignals(evidence?: { build?: string; lint?: string; techDebt?: string; tests?: Array<{ command?: string; passed?: number; failed?: number }> }): Partial<ScoringSignals> {
    if (!evidence) return {}
  ...
  return {
        ...
        async execute(args, context) {
          ...
          if (domainPack) {
            ...
            if (domainPack.name !== prevDomain) {
              mutableCurrentInjectDomain = domainPack
            }
          }
      ...
      // ── Batch delegate helper ──
      // Fan-out parallel execution based on A2A / Anthropic orchestrator-worker pattern.
...
{"files":[{"path":"src/x.ts","content":"..."}]} or {"noChanges":true}`
      ...
      // ── Web fetch with auto-RAG indexing ──
      agentic_fetch: registryTool("agentic_fetch", {
