...
/** Action verbs used for step generation */
const ACTION_VERBS = ["create", "implement", "build", "add", "configure", "setup", "write", "refactor"] as const
...
/** Words to skip during input schema inference */
const SKIP_WORDS = new Set(["this", "that", "with", "from", "into"])
...
function extractKeywords(goal: string): string[] {
  ...
  return [...new Set(words.filter(w => !STOP_WORDS.has(w.toLowerCase())))].slice(0, 8)
}
 ...
 */
function inferInputSchema(goal: string): Record<string, SchemaField> {
 ...
 */
function generateSteps(goal: string): SkillStep[] {
 ...
 */
function sampleValue(field: SchemaField): unknown {
...
function buildExpectedOutput(
  ...
  for (const [key, field] of Object.entries(outputSchema)) {
    ...
    output[key] = input[key] !== undefined ? input[key] : sampleValue(field)
  }
...
function autoGenerateTests(
  ...
  for (const [key, field] of Object.entries(inputSchema)) {
    happyInput[key] = sampleValue(field)
  }
  tests.push({
    name: "happy-path",
    input: happyInput,
    expectedOutput: outputSchema
      ? buildExpectedOutput(outputSchema, happyInput)
      : { result: "completed" },
  })
  ...
  for (const [key, field] of Object.entries(inputSchema)) {
    edgeInput[key] = field.required ? sampleValue(field) : null
  }
  tests.push({
    name: "edge-case",
    input: edgeInput,
    expectedOutput: outputSchema
      ? buildExpectedOutput(outputSchema, edgeInput)
      : { result: "completed" },
  })
 ...
 */
function findWeakestDimension(score: EvaluationScore): "correctness" | "schema" | "reusability" | "efficiency" {
...
function mutateSkill(
  ...
  const weakest = findWeakestDimension(score)
...
export class SkillImprover {
  ...
  async improve(
    ...
    const inputSchema = options?.inputSchema ?? inferInputSchema(goal)
    ...
    let testCases = autoGenerateTests(skill, inputSchema, outputSchema)
    let score = await evaluate(skill, testCases)
    let testResults = await runTests(skill, testCases)
    ...
    while (!accepted && iterations < maxIter) {
      const improved = mutateSkill(skill, score, testResults)
      if (!improved) break // can't improve further

      skill = improved
      testCases = autoGenerateTests(skill, inputSchema, outputSchema)
      score = await evaluate(skill, testCases)
      testResults = await runTests(skill, testCases)
      iterations++
  ...
  private generateSkill(
    ...
    const steps = generateSteps(goal)
    const keywords = extractKeywords(goal)
    const now = new Date().toISOString()

    // Try to generate DSL instructions if DslExecutor is available
    const logic = this.dslExecutor ? generateDSL(goal, inputSchema) : undefined
