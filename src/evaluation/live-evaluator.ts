  ...
  computeScore(sessionID?: string): LiveEvalScore {
    ...
    const dimensions: Record<string, LiveEvalDimension> = {
      taskSuccess: {
        ...
        confidenceInterval: this.computeCI(this.stepResults.map(s => s.success ? 1 : 0)),
      },
      errorRecovery: {
        ...
        confidenceInterval: this.computeCI(this.errorRecoveries.map(e => e.recovered ? 1 : 0)),
      },
      contextStability: {
        ...
        confidenceInterval: this.computeCI(this.navigations.map(n => n.focused ? 1 : 0)),
      },
      multiAgent: {
        ...
        confidenceInterval: this.computeCI(this.delegations.map(d => d.success ? 1 : 0)),
      },
      skillReuse: {
        ...
        confidenceInterval: this.computeCI(this.skillLookups.map(s => s.found ? 1 : 0)),
      },
    }
