...
export const AgenticEngine: Plugin = async (input, _options) => {
  ...
  return {
    tool: {
      agentic_plan: tool({
        description: "Create a structured execution plan. Can auto-decompose feature requests using built-in templates (create/implement, fix/bug, refactor, test). Call this FIRST for any multi-step task.",
        args: {
        ...
        async execute(args, context) {
          ...
          traceLogger.log({
            ...
            durationMs: 0,
            metadata: { errors, complexity: plan.complexity, autoDecomposed: subtasks !== args.subtasks },
          })
      ...
      agentic_verify: tool({
        description: "Run full verification: TypeScript compile + test suite. Includes error analysis on failure.",
        args: {
      ...
      agentic_pr: tool({
        description: "Generate a pull request description from the execution plan, all step results, and files changed. Use after completing all steps to create a ready-to-paste PR body.",
        args: {
        ...
        async execute(args, context) {
          ...
          // Auto-commit if git available (only if all steps passed)
          ...
          traceLogger.log({
            ...
            success: true,
            durationMs: 0,
            metadata: { commitHash: commitInfo?.hash },
          })
      ...
      agentic_delegate: tool({
        description: "Assign a task to a specialized agent role (architect/developer/qa/coordinator). The tool auto-suggests the best role based on the task description.",
        args: {
          ...
          role: tool.schema.enum(["architect", "developer", "qa", "coordinator"]).optional().describe("Target role (auto-detected if omitted)"),
      ...
      agentic_episodes: tool({
        ...
        async execute(args, context) {
          if (args.action === "search") {
            if (!args.query) return { output: "Provide a search query." }
            const episodes = episodicStore.search(args.query)
            ...
            output += episodes.map(e =>
              `- **${e.outcome === "success" ? "✅" : e.outcome === "partial" ? "⚠️" : "❌"} ${e.planGoal}**\n  Files: ${e.filesChanged.length} | Decisions: ${e.decisions.length} | ${e.timestamp.slice(0, 10)}`
            ).join("\n")
            return { output }
          }
