    ...
    tool: {
      ...
      agentic_auto: tool({
        ...
        async execute(args, context) {
          ...
          // 2. LLM-driven plan with timeout (fallback to template if LLM too slow)
          let subtasks: Subtask[] = []
          try {
            const intent = await Promise.race([
              planner.decomposeWithLLM(llmEngine, args.goal, summary),
              new Promise<never>((_, reject) => setTimeout(() => reject(new Error("LLM_TIMEOUT")), 25000)),
            ])
